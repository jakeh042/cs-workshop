...
q2 <- df %>%
  ...
  summarize(Avg = mean(`PBMCs (%CD45)`), StDev = sd(`PBMCs (%CD45)`)) %>%
...
q3 <- df %>%
  ...
  summarize(Avg = mean(Eosinophils)) %>%
  arrange(factor(`Disease Status`, levels=c("Normal", "Potential", "Affected")))
q3
```
...
```{r}
q4 <- df %>%
  group_by(`Tissue`, `Disease Status`) %>%
  filter(`CD8 Tem CD69+` == max(`CD8 Tem CD69+`)) %>%
  select(`donor`, `Tissue`, `Disease Status`, `CD8 Tem CD69+`) %>%
  arrange(factor(`Disease Status`, levels=c("Normal", "Potential", "Affected")))
q4
```
